...
//#region node_modules/.nitro/vite/services/ssr/assets/music.functions-D21Vgw1h.js
var MusicInput = objectType({
	prompt: stringType().min(1).max(1e3),
	lyrics: stringType().max(2e3).optional(),
...
var LyricsInput = objectType({
	prompt: stringType().min(1).max(1e3),
	genre: stringType().default("Lo-Fi"),
	mood: stringType().default("Relaxing")
});
var generateLyrics_createServerFn_handler = createServerRpc({
	id: "99bb93db3cdbf3b8f39cff9812e9a464e45d0def2c01751041cbf1fcd850e03d",
	name: "generateLyrics",
	filename: "src/lib/music.functions.ts"
}, (opts) => generateLyrics.__executeServer(opts));
var generateLyrics = createServerFn({ method: "POST" }).inputValidator((data) => LyricsInput.parse(data)).handler(generateLyrics_createServerFn_handler, async ({ data }) => {
	...
	if (geminiKey) {
		...
		return (await new GoogleGenAI({ apiKey: geminiKey }).models.generateContent({
...
function createAlgorithmicComposition(prompt, genre, mood, tempo, durationSeconds, providedLyrics) {
...
//#endregion
export { generateLyrics_createServerFn_handler, generateMusicTrack_createServerFn_handler };
